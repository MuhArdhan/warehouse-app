...
- **R11** (W9, 2026-09-20) Request serah terima ala-gudang = **Opsi A**: UI tipis di warehouse_app (Page `gudang_request` + picker endpoint read-only `requestable_work_orders`) memanggil `production_app.api.handover.create_request/cancel_request` **dari client** — production_app tetap satu-satunya penulis ringkasan WO (`custom_handover_material_request` + box), nol duplikasi logika; status endpoint dormant → **dikontrak konsumen luar**. Keputusan "jangan pindah dulu" (R2) TIDAK terlanggar: endpoint tidak dipindah; carve-out eksplisit — import Python production_app hanya diizinkan di file test/gate, bukan kode app. Validasi box andalkan server (atomic, fail-honest). Include: tombol Cancel + kolom "Adonan" di papan. Konfigurasi sekali di site: Manufacturing Settings `custom_default_handover_warehouse` = "Gudang Barang Jadi - ROPI" (sebelumnya kosong — endpoint memang belum pernah dikonfigurasi di site ini). Risiko kontrak tercatat: perubahan signature/field endpoint production_app mematahkan UI gudang — guard: gate W9 memanggil endpoint langsung, sehingga keretakan terlihat saat gate dieksekusi.
